...
import { Grid, Typography, Card, CardContent, CardMedia, Box, Grow } from '@mui/material';
...
const COMPWorkItems: React.FC<IWork.workProps> = ({filteredData,isDesktop}) => {
    ...
    return (
        <Grid container data-id='motion-div-filterWork' sx={STYLWork.workItems.itemsContainer}>
            {filteredData?.map((work: any, index: any) => (
                <Grow in={isDesktop} timeout={500} key={index}>
                    <Grid item data-id='card-container' sx={STYLWork.workItems.cardContainer}>
                        <Card sx={{ width: '100%', height: '100%', boxShadow: 'none' }}>
                            <Box data-id='img-box-container' sx={STYLWork.workItems.imgBox}>
                                <CardMedia
                                    component="img"
                                    height="140"
                                    width='100%'
                                ...
                                />
                                <Box data-id='image-item-tags' sx={STYLWork.workItems.imageTags}>
                                    {work.tags && work.tags.length > 0 ? work.tags[0] : 'No Tags'}
                                </Box>
                                <Box data-id='hover-box' sx={STYLWork.workItems.hoverBox}>
                                    <Box data-id='hover-box-content' sx={STYLWork.workItems.hoverContent}>
